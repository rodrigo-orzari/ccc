...
- Name: filters.spec.ts >> Dashboard Filter Validation >> Data & Analytics: Loads successfully without database empty warning
- Location: tests/filters.spec.ts:88:3

# Error details

```
Error: locator.click: Error: strict mode violation: locator('nav a').filter({ hasText: 'Data & Analytics' }) resolved to 2 elements:
    1) <a href="/?product=data-analytics" class="flex items-center gap-3 rounded px-2.5 py-2 text-xs font-bold transition-all border border-transparent text-[#737373] hover:text-black dark:hover:text-[#f7f8ff] opacity-70 hover:opacity-100 ">…</a> aka locator('a').nth(3)
    2) <a href="/?product=data-analytics" class="flex items-center gap-3 rounded px-2.5 py-2 text-xs font-bold transition-all border border-transparent text-[#737373] hover:text-black dark:hover:text-[#f7f8ff] opacity-70 hover:opacity-100 ">…</a> aka getByRole('link', { name: 'Data & Analytics' })

Call log:
  - waiting for locator('nav a').filter({ hasText: 'Data & Analytics' })
