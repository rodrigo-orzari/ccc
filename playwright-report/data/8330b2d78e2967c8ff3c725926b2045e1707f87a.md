...
- Name: filters.spec.ts >> Dashboard Filter Validation >> Serverless: Clearing all languages forces 0 results
- Location: tests/filters.spec.ts:21:3

# Error details

```
Error: locator.click: Error: strict mode violation: locator('nav a').filter({ hasText: 'Serverless' }) resolved to 2 elements:
    1) <a href="/?product=serverless" class="flex items-center gap-3 rounded px-2.5 py-2 text-xs font-bold transition-all border border-transparent text-[#737373] hover:text-black dark:hover:text-[#f7f8ff] opacity-70 hover:opacity-100 ">…</a> aka locator('a').filter({ hasText: 'Serverless' }).first()
    2) <a href="/?product=serverless" class="flex items-center gap-3 rounded px-2.5 py-2 text-xs font-bold transition-all border border-transparent text-[#737373] hover:text-black dark:hover:text-[#f7f8ff] opacity-70 hover:opacity-100 ">…</a> aka getByRole('link', { name: 'Serverless' })

Call log:
  - waiting for locator('nav a').filter({ hasText: 'Serverless' })
